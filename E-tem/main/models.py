...
class Recent(models.Model):
    user_num = models.OneToOneField(settings.AUTH_USER_MODEL, on_delete=models.CASCADE)
    ppt_recent = models.ManyToManyField(Powerpoint)

class Download_list(models.Model):
    user_num = models.OneToOneField(settings.AUTH_USER_MODEL, on_delete=models.CASCADE)
    ppt_download = models.ManyToManyField(Powerpoint)

class Color_tag(models.Model):
    color = models.CharField(max_length=100, db_index=True)
    ppt_tag = models.ManyToManyField(Powerpoint)
